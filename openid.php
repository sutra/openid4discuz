...
if($_GET['openid_mode'] == 'id_res'){ 	// Perform HTTP Request to OpenID server to validate key
	...
	if ($openid_validation_result == true){ 		// OK HERE KEY IS VALID
		...
		if (!$member_openid['uid']) {
			showmessage('你的 OpenID	(<a href="'.$openid->GetIdentity().'">'.$openid->GetIdentity().'</a>)尚未和论坛账号绑定。', 'logging.php?action=login');
		}
	...
	if ($openid->GetOpenIDServer()){
		$openid->SetApprovedURL('http://' . $_SERVER["HTTP_HOST"] .'/discuz/openid.php'. $_SERVER["PATH_INFO"]);  	// Send Response from OpenID server to this script
